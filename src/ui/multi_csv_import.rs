        ...
        match Database::open_writable(db_path) {
            Ok(mut db) => {
                ...
                for config in &self.csv_files {
                    ...
                    // Create indexes
                    for (column, create_index) in &config.create_indexes {
                        if *create_index {
                            if let Err(e) = db.create_index(&config.table_name, column) {
                                // Log but don't fail
                                eprintln!("Failed to create index on {}.{}: {}", config.table_name, column, e);
                            }
                        }
                    }
